...
import { acceptRewardRoot, createRewards, setRootUpdater, updateRewardRoot, setTimelock, getPendingRoot } from "src/lib/rewards.js";
...
const selectReward = async (dir: string, id: string | undefined) => {
  ...
  const reward = id === undefined ?
    await search({
      message: "Select a reward program to set timelock for",
      source: async (input) => {
        return rewards.filter(r => r.id.includes(input || "")).map(r => ({
          name: `${r.id} - (${r.name})`,
          value: r,
        }))
      }
...
export class CreateRewardsCommand extends Command {
  ...
  async execute() {
    ...
    let rewardProgram: z.infer<typeof MorphoRewardProgram> = {
      ...
      chainId: chain.id,
      start_timestamp: 1746547200,
      end_timestamp: 1747756800,
      production: this.prod,
      reward_amount: "204395150000000000000000",
...
export class ListRewardPrograms extends Command {
  ...
  async execute() {
    ...
    for (const reward of rewards) {
      console.log(`${reward.id}: (${reward.chainId}) ${reward.reward_amount} https://cornscan.io/address/${reward.urdAddress}`)
    }
  }
}
...
export class SetTimelock extends Command {
  ...
  async execute() {
    ...
    console.log(`updated timelock for urd ${reward.urdAddress} to ${this.timelock}. txn hash: ${txnhash}`)
  }
}
...
export class RepublishRoot extends Command {
  ...
  async execute() {
    ...
    const pendingRoot = await getPendingRoot(publicClient, getAddress(reward.urdAddress))
    ...
    console.log(`republished root for urd ${reward.urdAddress}. txn hash: ${txnhash}`)
  }
}
